/*! jQuery.file-upload - v0.1.0 - 2015-12-11 */
...
return _.noConflict=function(b){return a.$===_&&(a.$=Lb),b&&a.jQuery===_&&(a.jQuery=Kb),_},typeof b===za&&(a.jQuery=a.$=_),_})},{}],2:[function(a,b,c){(function(b){b.jQuery=a("jquery"),b.$=b.jQuery,a("./jquery.file-upload"),function(a){"use strict";a("#upload").fileUpload({progressCallBack:function(){var a=this.loadedSize,b=this.fileSize;this.$fileName.text(this.fileName),this.$transferSize.text(this.getDispFileSize(a)+" / "+this.getDispFileSize(b));var c=this.$progressBar.width()?this.$progressBar.width():1,d=a/b;if(1===d)this.$transferStatus.text("アップロード完了"),this.$stopButton.hide(),this.$deleteButton.show();else{var e=Math.floor(d*c);this.$progressBar.css("border-left-width",e+"px")}},doneCallBack:function(a){this.fileId=a.fileId},failCallBack:function(a,b,c){this.explicitStop||(409===a.status?this.$transferStatus.text("同名のファイルがあります"):this.$transferStatus.text("ファイルサイズの上限を超えています"),this.$stopButton&&this.$stopButton.hide(),this.$deleteButton&&this.$deleteButton.hide())},stopCallBack:function(){this.explicitStop=!0,this.ajax&&this.ajax.abort(),this.$transferStatus.text("アップロードを停止しました"),this.$stoppButton&&this.$stopButton.hide()},dragEnterCallBack:function(a){a.removeClass("dragLeave"),a.addClass("dragEnter")},dragLeaveCallBack:function(a){a.removeClass("dragEnter"),a.addClass("dragLeave")},dragOverCallBack:function(a){a.removeClass("dragLeave"),a.addClass("dragEnter")}})}(jQuery)}).call(this,"undefined"!=typeof global?global:"undefined"!=typeof self?self:"undefined"!=typeof window?window:{})},{"./jquery.file-upload":3,jquery:1}],3:[function(a,b,c){(function(c){!function(c){"use strict";"object"==typeof b&&"object"==typeof b.exports?b.exports=c(a("jquery"),window,document):c(jQuery,window,document)}(function(a,b,d,e){"use strict";var f,g={},h={url:"upload",deleteUrl:"upload",uploadButton:"#uploadButton",fileButton:"#file",uploadInfo:".uploadInfo",uploadInfoRowClass:"uploadInfoRow",stopButton:".stopButton",deleteButton:".deleteButton",dropArea:".dropArea",enableDragAndDrop:!0,progressCallBack:function(){},abortCallBack:function(){},doneCallBack:function(){},failCallBack:function(){},stopCallBack:function(){},deleteCallBack:function(){var b=this;this.fileId&&(a.ajax({url:g.deleteUrl+"/"+this.fileId,cache:!1,method:"delete"}).done(function(){b.$transferStatus.text("サーバーから削除しました")}).fail(function(a,b,c){}),this.$deleteButton&&this.$deleteButton.hide())},dragEnterCallBack:function(){},dragLeaveCallBack:function(){},dragOverCallBack:function(){},
// 行内の要素
rowHtml:'<div class="fileName"></div><div class="transferSize"></div><div class="transferStatus"><div class="progressBar"></div></div><div class="operation"><input type="button" class="stopButton" value="停止"><input type="button" class="deleteButton hide" value="削除"></div>',rowEl:{$fileName:".fileName",$transferSize:".transferSize",$transferStatus:".transferStatus",$progressBar:".progressBar"}},i={type:"post",processData:!1,contentType:!1,// 送信するデータをFormDataにする場合、こうしないといけない。
cache:!1,dataType:"json"},j=function(b,c){
// コンストラクタ
this.$upload=c,this.$uploadInfo=this.$upload.find(g.uploadInfo),this.$uploadInfoRow=a("<div></div>").addClass(g.uploadInfoRowClass),this.$uploadInfo.append(this.$uploadInfoRow),this.fileName=b.name,this.fileSize=b.size,this.loadedSize=0,
// stopButtonを押して停止したかどうか
this.explicitStop=!1,
// 行内の要素
this.$uploadInfoRow.html(g.rowHtml);for(var d in g.rowEl)g.rowEl.hasOwnProperty(d)&&(this[d]=this.$uploadInfoRow.find(g.rowEl[d]));this.formData=new FormData,this.formData.append("file",b);
// イベント処理
var e=this;this.$stopButton=this.$uploadInfoRow.find(g.stopButton),this.$deleteButton=this.$uploadInfoRow.find(g.deleteButton),this.$stopButton&&this.$stopButton.on("click",function(){g.stopCallBack.call(e)}),this.$deleteButton&&this.$deleteButton.on("click",function(){g.deleteCallBack.call(e)})};j.prototype.start=function(){var b=this,d=a.extend({xhr:function(){var a=new c.XMLHttpRequest;
// アップロードの進捗
// 中止の場合
return a.upload.addEventListener("progress",function(a){b.progress.call(b,a)},!1),a.addEventListener("abort",function(a){b.abort.call(b,a)},!1),a},url:g.url,data:b.formData},i);b.ajax=a.ajax(d).done(function(a){g.doneCallBack.call(b,a),b.ajax=null}).fail(function(a,c,d){g.failCallBack.call(b,a,c,d),b.ajax=null})},/**
   * アップロード処理中のコールバック。<br>
   * アップロードしたサイズを記録する。
   * @param event イベント
   */
j.prototype.progress=function(a){a.lengthComputable&&(this.loadedSize=a.loaded,this.loadedSize>this.fileSize&&(this.loadedSize=this.fileSize)),g.progressCallBack.call(this,a)},/**
   * アップロードが中断された時のコールバック
   * @param event イベント
   */
j.prototype.abort=function(a){g.abortCallBack.call(this,a)},/**
   * アップロードを明示して停止する際のメソッド
   */
j.prototype.stop=function(){this.ajax&&(this.explicitStop=!0,this.ajax.abort()),g.call.stopCallBack(this)},/**
   * 表示用のファイルサイズ
   * @param plainSize 実際のファイルサイズ（バイト）
   * @returns {string} 表示用のファイルサイズ
   */
j.prototype.getDispFileSize=function(a){for(var b=["B","KB","MB","GB","TB"],c=parseInt(a,10),d=0;d<b.length&&!(1e3>c);d++)c/=1024;return c=c===Math.floor(c)?Math.floor(c):c.toPrecision(3),c+b[d]};var k=function(){var b=a(this),c=b.find(g.dropArea);c.on("dragenter",function(a){a.preventDefault(),a.stopPropagation(),g.dragEnterCallBack.call(this,c)}),c.on("dragleave",function(a){a.preventDefault(),a.stopPropagation(),g.dragLeaveCallBack.call(this,c)}),c.on("dragover",function(a){a.preventDefault(),a.stopPropagation(),g.dragOverCallBack.call(this,c)}),c.on("drop",function(a){a.preventDefault(),a.stopPropagation(),l(a.originalEvent.dataTransfer.files,b)})},l=function(a,b){for(var c=0;c<a.length;c++){var d=new j(a[c],b);d.start()}};a.fn.fileUpload=function(b){g=a.extend(h,b);var c=this;f=a(g.fileButton),a(g.uploadButton).on("click",function(){f.trigger("click")}),g.enableDragAndDrop&&k.call(this),f.on("change",function(){l(f[0].files,c)})}})}).call(this,"undefined"!=typeof global?global:"undefined"!=typeof self?self:"undefined"!=typeof window?window:{})},{jquery:1}]},{},[3,2]);
//# sourceMappingURL=jquery.file-upload.dev.min.js.map